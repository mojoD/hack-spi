...
# Sonic Pi Midithru program

####### midithru statement in Sonic Pi must be the first statement ##########
...
def midithru(midiInstrument)
  ...
  when "instrument"
    parmArray.each do |cmdLine|
      ...
      when "gmPathDir"
        puts "gmPathDir: " + cmdVal.to_s
        instgmPathDir = "C:/" + cmdVal[1...-1].to_s + "/"
        puts "instgmPathDir: " + instgmPathDir.to_s
      when "transpose"
        transpose = cmdVal.to_i
      end       
    end
  ...
  in_thread do
    ...
    while udpActive do
      ...
      midiEvent =  midiEventArray[0][0...midiEventArray[0].length]
      ...
      if timestamp.to_i > oldtimestamp.to_i
        ...
        if synthType.to_s == "use_synth"
          if transpose == 0            
              play midinote.to_i, amp: amp.to_f, release: release.to_f
          else
              play midinote.to_i + transpose.to_i, amp: amp.to_f, release: release.to_f
          end
        end
