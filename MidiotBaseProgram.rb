...
def samplePlayer (i, duration, gmPathDir, sampleArray, midiArray, presetSettings, attack=0.01, decay=0.3, sustain=0.5, releaseTime=0, lpfFreqCutoff, hpfFreqCutoff, amp, bpm, panIn, midiFlag)
  ...
  #puts "noteDurationInFrames: " + noteDurationInFrames.to_s
  #puts "sampleFrames: " + sampleFrames.to_s
...
def midiSamplePlayer (gmPathDir, midiArray, sampleArray, presetSettings, attack, decay, sustain, releaseTime, lpfFreqCutoff, hpfFreqCutoff, amp, bpm, pan, glIn, glFunc)
  loopFor = midiArray[0]
...
def instrument(midiIn, duration: 1.0, gmBank: "000", gmPatch: "000", gmPathDir: "C:/Users/Michael Sutton/Midiloop/default/", amp: 1.0, attack: 0.01, decay: 0.3, sustain: 0.5, release: 1.0, lpf: 128, hpf: 1, bpm: 60, pan: 0.0, glIn: "none", glFunc: "none")  
  ...
  if gmPathDir[0] == "'" 
    sampleFileName = testPathDir + sampleToPlay + ".pisf"
    sampleArray = getSample(sampleFileName, testPathDir)    
  else
  ...
  if midiIn.instance_of? Array  
    if midiIn[0].instance_of? String
      midiSamplePlayer gmPathDir, midiIn, sampleArray, presetSettings, attack, decay, sustain, release, lpfFreqCutoff, hpfFreqCutoff, amp, bpm, pan, glIn, glFunc
    end
  end
  ##### 
  ...
  if midiIn.instance_of? Symbol
    ...
    midiInStr = midiIn
    if gmPathDir[0] == "'" 
      samplePlayer midiIn.to_i, duration, testPathDir, sampleArray, midiArray, presetSettings, attack, sustain, decay, release, lpfFreqCutoff, hpfFreqCutoff, amp, bpm, pan, "N"
    else
