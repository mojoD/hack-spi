...
def displayShape
  ...
  $drawArray.each do |drawElement|
    ...
    if drawElement.action.to_s != "kill"
      #puts "drawElement.shape: " + drawElement.shape.to_s
...
def displayString(textIn)
  ...
  if textIn.action != "kill"
    ...
    if textIn.translaterX == 0.0 && textIn.translaterY == 0.0 && textIn.translaterZ == 0.0
      #do nothing
    else      
      textIn.translateX = textIn.translateX + textIn.translaterX; textIn.translateY = textIn.translateY + textIn.translaterY; textIn.translateZ = textIn.translateZ + textIn.translaterZ;
    end      
    ...
    glRasterPos3f(textIn.p1x.to_f, textIn.p1y.to_f, textIn.p1z.to_f)
...
def displayLine(lineIn)
  ...
  if lineIn.action != "kill"
    ...
    if lineIn.scaleX == 1.0 && lineIn.scaleY == 1.0 && lineIn.scaleZ == 1.0
      ...
      scaleObj(lineIn.scaleX, lineIn.scaleY, lineIn.scaleZ)
    end  
    glBegin(GL_LINES)
...
def canvas(argsIn)
  ...
  glutPositionWindow($posX.to_f, $posY.to_f)
  glutReshapeWindow($screenX.to_f, $screenY.to_f)
  glutPostRedisplay()
...
class DrawObject
  ...
  attr_accessor :sid, :clear, :action, :killtimer, :killcntr, :text, :font, :texture
  ...
  def initialize(dataIn)
    ...
    text = ''; font = 'GLUT_BITMAP_9_BY_15'; texture = ""; \
  ...
  cmdArray.each do |cmdLine|
    ...
    @text = text; @font = font; @texture  = texture;
...
#commandparser("canvas: 'Y', blend: 'N', screenX: 1920, screenY: 1080, posX: 0, posY: 5, title: 'Visualizer', bkr: 0.0, bkg: 0.0, bkb: 0.0, bka: 1.0")
...
visualizer
sleep 1
#commandparser("sid: txt1, shape: text, text: 'This is a test', p1x: -1.1, p1y: 0.1, p1z: 0.0, font: 'GLUT_BITMAP_TIMES_ROMAN_24', scr: 1.0, scg: 0.0, scb: 0.0") 
#sleep 8
