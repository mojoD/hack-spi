...
outArray = []
outArray2 = [] # adsr, reverb and chorus settings by instrument
sampleName = ""
instrumentName = ""
lowKey = ""
highKey = ""
...
instrumentArray.each do |instrumentLine|
    ...
    if instrumentLine.include? "GZ_sustainVolEnv"
        sustain = instrumentLine.partition("=").last.to_s
    end
    if instrumentLine.include? "GZ_releaseVolEnv"
        release = instrumentLine.partition("=").last.to_s
    end    
    if instrumentLine.include? "GZ_decayVolEnv"
        decay = instrumentLine.partition("=").last.to_s
    end  
    if instrumentLine.include? "GZ_reverbEffectsSend"
        reverb = instrumentLine.partition("=").last.to_s
    end  
    if instrumentLine.include? "GZ_chorusEffectsSend"
        chorus = instrumentLine.partition("=").last.to_s
    end  
    ...
    if instrumentLine.include? "Z_overridingRootKey"
        ...
        for i in lowKeyint..highKeyint          
            ...
            outrec2 = instrumentName.to_s[0..instrumentName.length-2] + "," + sustain.to_s[0..sustain.length-2] + "," + decay.to_s[0..decay.length-2] + "," + release.to_s[0..release.length-2] + "," + reverb.to_s[0..reverb.length-2] + "," + chorus.to_s[0..chorus.length-2] 
            logfile.puts "outrec2: " + outrec2.to_s
            outArray2.push(outrec2) 
        end
    end
...
outArray.each do |outRec|
    logfile.puts "xoutRec: " + outRec.to_s
    currentInstrument = outArray[i].split(",")[0]
    #currentInstrument = outArray[i].split(",")[3]    
    logfile.puts "curInst:  " + currentInstrument.to_s
    logfile.puts "lastInst: " + lastInstrument.to_s
    if currentInstrument == lastInstrument
        singleInstrumentArray.push outRec
    else
        logfile.puts "different instrument"
        fixArray = []
        fixArray2 = []

        singleInstrumentArray.each do |testline|
            testFileName = directoryname + "/" + testline.split(",")[3] + ".wav"
            logfile.puts "testFileName: " + testFileName.to_s
            fileTest = File.file?(testFileName)
            if fileTest == true
                logfile.puts "found file"
                fixArray.push(testline) 
            end
        end
        ...
        currentnotetest = ""
        prevnotenumtest = ""
        fixArray.each do |testline2|
            logfile.puts "recCnt: " + recCnt.to_s
            logfile.puts "testline2: " + testline2.to_s
            if recCnt.to_i == 0
                logfile.puts "recCnt = 0"
                fixArray2.push testline2
                prevnotenumtest = testline2.to_s.split(",")[1]
            else
                currentnotetest = testline2.to_s.split(",")[1]
                logfile.puts "currentnotetest: " + currentnotetest.to_s
                logfile.puts "prevnotenumtest: " + prevnotenumtest.to_s 
                if currentnotetest.to_i <= prevnotenumtest.to_i
                    logfile.puts "record to insert: " + testline2.to_s
                    logfile.puts "record to delete: " + fixArray2[currentnotetest.to_i-1].to_s
                    if fixArray2[currentnotetest.to_i-1].to_s != ""
                        fixArray2.delete_at(currentnotetest.to_i-1)
                        fixArray2.insert(currentnotetest.to_i-1,testline2)
                    end
                end         
                if currentnotetest.to_i > prevnotenumtest.to_i
                    fixArray2.push testline2
                    prevnotenumtest = currentnotetest
                end
            end
            recCnt += 1
        end
        singleInstrumentArray = fixArray2
        logfile.puts "singleInstrumentArray: " + singleInstrumentArray.to_s

        # set up output file
        filename = directoryname + "/" + lastInstrument + ".pisf" 
        puts filename.to_s
        outfile = File.new(filename,"w+")        
        logfile.puts "singleInstrumentArray[0]:  " + singleInstrumentArray[0].to_s
        ...
        singleInstrumentArray.each do |instrumentRec|
            ...
            outfile.puts newrec
            logfile.puts "newrec: " + newrec.to_s 
            singleInstrumentArrayPos += 1  
        end
        logfile.puts "closing file and resetting array"
...
puts filename.to_s
outfile = File.new(filename,"w+")        
logfile.puts "filename: " + filename.to_s
noteNum = singleInstrumentArray[0].split(",")[1]
singleInstrumentArrayPos = 0 
#outrec = ""
singleInstrumentArray.each do |instrumentRec|
    logfile.puts "instrumentRec: " + instrumentRec.to_s
    recToMatch = instrumentRec
    noteToFind = instrumentRec.split(",")[1]
    matchArrayPos = 0
    recMatch = "N" 
    ...
    testInstrument = instrumentRec.split(",")[3]
    #testFileName = directoryname + "/" + lastInstrument + ".wav"
    testFileName = directoryname + "/" + testInstrument + ".wav"    
    logfile.puts "testFileName in pisf: " + testFileName.to_s
    fileTest = File.file?(testFileName)
    logfile.puts "fileTest in pisf: " + fileTest.to_s
    if File.file?(testFileName)
        logfile.puts "rec to output"
        logfile.puts "newrec: " + newrec.to_s
        outfile.puts newrec
        singleInstrumentArrayPos += 1        
    end
...
# Fix up preset Array for missing before and after notes
testProgram = presetInArray[0].split(",")[1]
if testProgram.to_s != "000"
    puts "not 000"
end
...
pisfDir = ARGV[1]
pisfOutArray = []
logfile.puts "pisfDir: " + pisfDir.to_s
Dir.foreach(pisfDir) do |pisfFile|
    pisfOutArray = []
    if pisfFile.include? ".pisf"
        logfile.puts "psfiFile: " + pisfFile.to_s
        pisfFileIn = pisfDir + "/" + pisfFile
        logfile.puts "pisfFileIn: " + pisfFileIn.to_s 
        f = File.open(pisfFileIn, 'r') 
            pisfInArray = f.readlines
        f.close
        logfile.puts "pisfRec: " + pisfInArray[0].to_s
        logfile.puts "Arraysize: " + pisfInArray.length.to_s
        if pisfInArray.length > 1
            logfile.puts "pisfRecLength: " + pisfInArray[0].length.to_s
            logfile.puts "note: " + pisfInArray[0].split(",")[1].to_s
            if pisfInArray[0].split(",")[1] != 0
                logfile.puts "note does not start at 0"
                #add filler notes at beginning
                for i in 0..pisfInArray[0].split(",")[1].to_i-1
                    #build new records
                    out1 = pisfInArray[0].split(",")[0].to_s
                    out2 = i.to_s 
                    out3 = pisfInArray[0].split(",")[2].to_s
                    out4 = pisfInArray[0].split(",")[3].to_s
                    out5 = pisfInArray[0].split(",")[4].to_s
                    out6 = pisfInArray[0].split(",")[5].to_s
                    out7 = pisfInArray[0].split(",")[6].to_s
                    out8 = pisfInArray[0].split(",")[7].to_s
                    out9 = pisfInArray[0].split(",")[8].to_s
                    out10 = pisfInArray[0].split(",")[9].to_s
                    out11 = pisfInArray[0].split(",")[10].to_s
                    out12 = "0"
                    outRec = out1 + "," + out2 + "," + out3 + "," + out4 + "," + out5 + "," + out6 + "," + out7 + "," + out8 + "," + out9 + "," + out10 + "," + out11 + "," + out12
                    logfile.puts "outRec: " + outRec.to_s
                    pisfOutArray.push outRec
                end 
                pisfInArray.each do |inRec|
                    logfile.puts "inrec: " + inRec.to_s
                    out1 = inRec.split(",")[0].to_s
                    out2 = inRec.split(",")[1].to_s 
                    out3 = inRec.split(",")[2].to_s
                    out4 = inRec.split(",")[3].to_s
                    out5 = inRec.split(",")[4].to_s
                    out6 = inRec.split(",")[5].to_s
                    out7 = inRec.split(",")[6].to_s
                    out8 = inRec.split(",")[7].to_s
                    out9 = inRec.split(",")[8].to_s
                    out10 = inRec.split(",")[9].to_s
                    out11 = inRec.split(",")[10].to_s
                    out12 = "0"
                    outRec = out1 + "," + out2 + "," + out3 + "," + out4 + "," + out5 + "," + out6 + "," + out7 + "," + out8 + "," + out9 + "," + out10 + "," + out11 + "," + out12
                    logfile.puts "outRec: " + outRec.to_s
                    pisfOutArray.push outRec
                    #pisfOutArray.push inRec
                end
                logfile.puts "pisfLastRec: " + pisfOutArray.last.to_s
                copyRec = pisfOutArray.last
                lastRecnum = pisfOutArray.last.split(",")[1].to_i+1
                logfile.puts "lastRecnum: " + lastRecnum.to_s

                for i in lastRecnum..127
                    #puts "post processing"
                    out1 = copyRec.split(",")[0].to_s
                    out2 = i.to_s 
                    out3 = copyRec.split(",")[2].to_s
                    out4 = copyRec.split(",")[3].to_s
                    out5 = copyRec.split(",")[4].to_s
                    out6 = copyRec.split(",")[5].to_s
                    out7 = copyRec.split(",")[6].to_s
                    out8 = copyRec.split(",")[7].to_s
                    out9 = copyRec.split(",")[8].to_s
                    out10 = copyRec.split(",")[9].to_s
                    out11 = copyRec.split(",")[10].to_s
                    out12 = "0"
                    outRec = out1 + "," + out2 + "," + out3 + "," + out4 + "," + out5 + "," + out6 + "," + out7 + "," + out8 + "," + out9 + "," + out10 + "," + out11 + "," + out12
                    logfile.puts "outRec: " + outRec.to_s
                    pisfOutArray.push outRec
                end
                File.delete(pisfFileIn)
                outPisfFile = File.new(pisfFileIn,"w+")                  
                pisfOutArray.each do |outRec|
                    if outRec.split(",").last.to_i == 0
                        outPisfFile.puts outRec
                    end
                end
                outPisfFile.close                
            end
...
#build a file with the settings for envelopes, reverb, chorus, etc
instrumentIn = ""
filename = directoryname + "/.params.info" 
outfile2 = File.new(filename,"w+")
outArray2.each do |outrec2|
    logfile.puts "outrecArray2: " + outrec2.to_s
    instrumentTest = outrec2.split(",")[0]
    sustain = outrec2.split(",")[1]
    decay = outrec2.split(",")[2]
    release = outrec2.split(",")[3]
    reverb = outrec2.split(",")[4]
    chorus = outrec2.split(",").last
    if instrumentTest != instrumentIn
        instrumentIn = instrumentTest
        outfile2.puts outrec2
    end
end
outfile2.close
