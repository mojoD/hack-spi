...
outArray.each do |outRec|
    ...
    if currentInstrument == lastInstrument
        ...
        fixArray.each do |testline2|
            if  testline2.to_s.include? "Strings,1,"
                logfile.puts "Mike: " +  testline2.to_s.split(",")[0]
            end
...
pisfDir = ARGV[1]
pisfOutArray = []
#puts "pisfDir: " + pisfDir.to_s
Dir.foreach(pisfDir) do |pisfFile|
    pisfOutArray = []
    if pisfFile.include? ".pisf"
        #puts "psfiFile: " + pisfFile.to_s
        pisfFileIn = pisfDir + "/" + pisfFile
        #puts "pisfFileIn: " + pisfFileIn.to_s 
        f = File.open(pisfFileIn, 'r') 
            pisfInArray = f.readlines
        f.close
        #puts "pisfRec: " + pisfInArray[0].to_s
        #puts "Arraysize: " + pisfInArray.length.to_s
        if pisfInArray.length > 1
            #puts "pisfRecLength: " + pisfInArray[0].length.to_s
            #puts "note: " + pisfInArray[0].split(",")[1].to_s
            if pisfInArray[0].split(",")[1] != 0
                #puts "note does not start at 0"
